...
# 530     | 03/05/17 | T. Webb    | Added -p (physical_server_abbrev) 
#         |          |            | ..It should be -H for hostname but that was already taken!
# 540     | 08/05/17 | T. Webb    | Changes to the -j flag
#############################################################################################
...
typeset TEMPFILE7="${TEMP_DIR}/amchecks7.txt"
typeset THISDB
...
##    typeset -u UPPER_SID=${SID}
    typeset -u UPPER_SID=`echo ${SID} | cut -d'+' -f1-2`
    ...
    PARAM1=`echo ${SID} | cut -d'+' -f4`
    if [[ ${PARAM1} == 'null' ]] || [[ ${PARAM1} == ${SID} ]]
    then
        PARAM1=' '
    fi

    PARAM2=`echo ${SID} | cut -d'+' -f5`
    if [[ ${PARAM2} == 'null' ]] || [[ ${PARAM2} == ${SID} ]]
    then
        PARAM2=' '
    fi
...
#        PRINT_SID="Running checks on ${UPPER_SID} (hosted on ${HOSTINFO})"
    ...
    then
        ...
        then
            if [[ `echo ${SERVERTITLE} | wc -c` -gt 1 ]]
            then
                echo "${SERVERTITLE}" >> ${OUTPUT_BUFFER}  
                echo "${SERVERTITLE}" >> ${MAIL_BUFFER} 
                echo >> ${OUTPUT_BUFFER} 
                echo >> ${MAIL_BUFFER}
            fi
        fi
    ...
    then
        ...
        then
            ...
            if [[ ! -z ${SCRIPT} ]]
            then
                echo "start ${AMCHECK_DIR}/${SCRIPT} ${PARAM1} ${PARAM2}" > ${SCRIPTFILE}
                RET=$?
            elif [[ ${USE_TNSNAMES} == 'Y' ]]
            then
...
while getopts a:A:bcCd:De:fh:Hj:k:K:mp:s:STt:x o
do      case "$o" in
...
then
    print -- "${THISSCRIPTNAME}: Script sets (-j) and database names (-d) parameters are currently incompatable. Please inspect your parameters: \n${USAGE}" 
    exit 50
...
if [[ ! -z ${SCRIPT_ID} ]]
then
    UNFORMATTED_STRING=`sqlplus -s ${AMCHECK_TNS} <<- SQL065
	set pages 0
	SET FEEDBACK OFF
	SELECT  NVL(j.am_database_where_string,'BANANA') || '+' || NVL(j.title ,'APPLE')
	FROM    amo.am_script_set j
	WHERE   j.set_id = '${SCRIPT_ID}'
	AND     j.disabled <> 'Y';
	exit;
SQL065
`
    WHERE_ADDITION2=`echo ${UNFORMATTED_STRING} | cut -d'+' -f1`
    SCRIPT_SET_TITLE=`echo ${UNFORMATTED_STRING} | cut -d'+' -f2-`
    if [[ ${WHERE_ADDITION2} != "BANANA" ]]
    then
        WHERE_ADDITION2=" AND d.${WHERE_ADDITION2}"
    else
        WHERE_ADDITION2=' '
    fi

    if [[ ${SCRIPT_SET_TITLE} != "APPLE" ]] && [[ ! -z ${SCRIPT_SET_TITLE} ]] 
    then
        MAIL_TITLE=${SCRIPT_SET_TITLE}
    fi

    sqlplus -s ${AMCHECK_TNS} <<- SQL070 > ${TEMPFILE6}
        set pages 0
	SET FEEDBACK OFF
	SELECT  d.database_name || '+' || z.script_name || '+' || TRANSLATE(NVL(j.title,j.set_id),' ','_') || '+' 
		|| NVL(z.param1,'null') || '+' || NVL(z.param2,'null') AS database_name
	FROM    amo.am_database d, amo.am_server s, amo.am_script_set j, amo.am_scripts z, amo.am_script_set_component c
	WHERE   c.script_id = z.script_id
	AND     c.database_name = d.database_name
	AND     d.server = s.server
	AND     j.set_id = '${SCRIPT_ID}'
        AND     j.set_id = c.set_id
	AND     d.disabled <> 'Y'
	AND     s.disabled <> 'Y'
        AND     c.disabled <> 'Y'
	AND     j.disabled <> 'Y'
	...
	AND     s.disabled <> 'Y'
	AND     j.disabled <> 'Y'
	AND     c.disabled <> 'Y' ${WHERE_ADDITION2}
	ORDER BY database_name ASC;
        exit;
	SQL070
        cat ${TEMPFILE6} | sed '/^$/d' | while read SID
        do
##          echo " debug f_run ${SID}"
          f_run ${SID}
        done
elif [[ ! -z ${ONE_DB} ]]
then
...
--	SELECT d.database_name || ' on server ' || s.physical_server_abbrev || CASE WHEN s.cluster_name IS NULL THEN NULL ELSE ' (cluster: ' || s.cluster_name || ')' END AS cluster_name FROM   amo.am_database d, amo.am_server s WHERE  d.server = s.server AND d.disabled <> 'Y' AND s.disabled <> 'Y' ${WHERE_ADDITION} ORDER BY d.run_order ASC, d.database_name ASC;
	...
	FROM  (SELECT   d.run_order,
		...
		FROM    amo.am_database d, 
			amo.am_server s
		WHERE   1=1
		AND     UPPER(s.physical_server_abbrev) LIKE '${HOSTNAME}'
		AND     d.disabled <> 'Y'
