...
typeset USAGE="Incorrect Usage. TO FIX run: ${THISSCRIPTNAME} -A (alerting enabled) -a mail-address -h title -S nn (Smart mail) -f (force mail) -l label -s (short output) -c (cron mode) -k (inline colour) -K (attachment colour) -t (use tnsnames) -m (mail) -p (only tnsping) -P (server ping only) -v (verbose)"
...
while getopts Aa:cefh:k:K:l:mpPsS:tT:v o
do      case "$o" in
        A)      ALERT_IND='Y';;
...
then
    ...
    then
    	...
    	set pages 0
    	set feedback off
    	SELECT distinct physical_server FROM amo.am_server WHERE disabled <> 'Y' AND ping_disabled <> 'Y' ORDER BY 1;
    	exit;
SQL090
