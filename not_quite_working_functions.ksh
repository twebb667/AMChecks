...
#    echo "debug FILE_EXTENSION ${FILE_EXTENSION}"
	...
	AMC_LINE3="Content-Transfer-Encoding: 7bit"
        ...
        echo
        # debug 1 CONTENT_TYPE="Content-Type: text/html; charset=ISO-8859-15"
    fi
...
cat > /tmp/amchecks/bibble <<- EOM
	...
	EOM
        if [[ "${INLINE_FILE}" != "null" ]]
        then
	    echo "${AMC_LINE1}" >> /tmp/amchecks/bibble
        fi
	echo "${IMAGE_HTML6}" >> /tmp/amchecks/bibble
	echo "${IMAGE_HTML7}" >> /tmp/amchecks/bibble
	echo "${IMAGE_HTML8}" >> /tmp/amchecks/bibble
        echo " " >> /tmp/amchecks/bibble
	echo "${IMAGE_HTML9}" >> /tmp/amchecks/bibble

    /usr/sbin/sendmail -t < /tmp/amchecks/bibble

    echo "file being sent is /tmp/amchecks/bibble"
